...
resource "aws_launch_template" "ecs_template" {
  name_prefix   = "ecs-jenkins-"
  image_id      = "ami-0f79cea3ec3565580"
  instance_type = "t3.large"

  vpc_security_group_ids = [var.aws_security_group_allow_all_traffic_id]
  ...
  user_data = base64encode(<<-EOF
    #!/bin/bash
    echo ECS_CLUSTER=${aws_ecs_cluster.app-cluster.name} >> /etc/ecs/ecs.config
  EOF
  )
...
resource "aws_autoscaling_group" "ecs_asg" {
  ...
  max_size            = 3
  min_size            = 1
  vpc_zone_identifier = [var.aws_subnet_private1_id, var.aws_subnet_private2_id]
  launch_template {
...
resource "aws_ecs_task_definition" "jenkins_master_with_docker" {
...
resource "aws_ecs_service" "jenkins" {
  ...
  task_definition = aws_ecs_task_definition.jenkins_master_with_docker.arn
  launch_type     = "EC2"
  desired_count   = 1
